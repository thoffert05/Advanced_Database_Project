...
Then spark_ingest.py is called which runs a script in Yarn to read the CSV file from HDFS and write
it into HDFS parquets as partitions split up by day so that it makes clean up easier
...
cruise_schema: this is the schema used to ingest the cruise ship stats file, the schema is provided to speed
               ...
               again based on that schema, though for this file it wouldn't be a problem since it is small
...
def sanitize_csv(bucket_name,filename,hdfs_client,hdfs_path,schema,file_index,total_files):
         ...
         #prevent divide by 0 incase the CSV has less than 100 lines
         if one_percent==0:
            one_percent=1
